...
from .mesh import get_scale_and_shift, scale_and_shift_triangle, get_shift
...
def voxelize(file_path, t_matrix):
    ...
    for triangle in list_of_triangles:
        progress_counter += 1
        progress_bar(progress_counter, len(list_of_triangles), prefix="Voxelize: ")

        shifted_triangle = []
        for point in triangle:
            new_p = np.matmul(t_matrix, np.append(point, 1))
            shifted_triangle.append(new_p[:-1])

        (vertex_1, vertex_2, vertex_3) = shifted_triangle
...
def voxelize_nomatrix(file_path, scale):
    ...
    shift, triangle_count = get_shift(list_of_triangles)
    progress_counter = 0
    voxels = set()
    bounding_box = BoundaryBox()
    for triangle in list_of_triangles:
...
def old_voxelize(file_path, resolution, progress_bar=None):
    """
    @type file_path: str
    @type resolution: int
    ...
    for triangle in list_of_triangles:
        ...
        (vertex_1, vertex_2, vertex_3) = scale_and_shift_triangle(triangle, 1, [0, 0, 0])
        bounding_box.from_vertexes(vertex_1, vertex_2, vertex_3)
        voxels.update(get_intersecting_voxels_depth_first(vertex_1, vertex_2, vertex_3))
    center = bounding_box.get_center()
    while len(voxels) > 0:
