...
class DCMToTensorNoGrnd(object):
    ...
    def __call__(self, pic, grnd):
        ...
        Returns:
            Tensor: Converted image.
        """
        return F.to_tensor(pic.copy()).float().div(4095), torch.from_numpy(grnd.copy()[None, :, :])
...
class Interpolate(object):
    ...
    def __call__(self, img, grnd):
        ...
        Returns:
            PIL Images: Rescaled image and segmentation mask.
        """
        return torch.nn.functional.interpolate(img, self.size,
                                               mode=self.interpolation), torch.nn.functional.interpolate(
            grnd, self.size, mode='nearest')
